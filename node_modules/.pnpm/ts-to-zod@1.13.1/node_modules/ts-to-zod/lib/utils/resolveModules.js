...
const moduleToPrefix = (declarationNames) => (context) => (sourceFile) => {
    const prefixInterfacesAndTypes = (moduleName) => (node) => {
        ...
        if (typescript_1.default.isTypeAliasDeclaration(node)) {
            return typescript_1.factory.updateTypeAliasDeclaration(node, node.decorators, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.typeParameters, typescript_1.default.isTypeLiteralNode(node.type)
                ? typescript_1.factory.updateTypeLiteralNode(node.type, typescript_1.default.visitNodes(node.type.members, prefixInterfacesAndTypes(moduleName)))
                : typescript_1.default.isTypeReferenceNode(node.type)
                    ? typescript_1.factory.updateTypeReferenceNode(node.type, node.type.typeName, typescript_1.default.visitNodes(node.type.typeArguments, prefixInterfacesAndTypes(moduleName)))
                    : node.type);
        }
        if (typescript_1.default.isInterfaceDeclaration(node)) {
            return typescript_1.factory.updateInterfaceDeclaration(node, node.decorators, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.typeParameters, node.heritageClauses, typescript_1.default.visitNodes(node.members, prefixInterfacesAndTypes(moduleName)));
        }
        if (typescript_1.default.isEnumDeclaration(node)) {
            return typescript_1.factory.updateEnumDeclaration(node, node.decorators, node.modifiers, typescript_1.factory.createIdentifier((0, case_1.pascal)(moduleName) + (0, case_1.pascal)(node.name.text)), node.members);
        }
    ...
    return typescript_1.default.visitNode(sourceFile, flattenModuleDeclaration);
};
